...
ldf1[[2]][,3] <- with(ldf1[[2]], !(regexpr("mean", V2, ignore.case=TRUE)<0 & 
                                     regexpr("std", V2, ignore.case=TRUE)<0 ))
...
# replace column names to be appropriate
#   feature names were changed to eliminate "-" and "()"
names(tidyset) <- c("Subject", gsub(",", "", gsub("-", ".", gsub("\\(", "", gsub("\\)", "", feat[,2])))), "Activity")
